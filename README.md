...
```text
.
├── retrieval/           # Scripts and results for retrieval-based evaluation
├── DataCopying/         # Z_U metric calculation (Memorization Test)
├── fpd_eval/            # Fréchet Point Distance (FPD) calculation
├── hy3dshape/           # Model training (Vecset Diffusion + Hunyuan3D)
├── data_curation/       # Data filtering and processing pipeline
└── mvcaption/           # Multi-view captioning strategy
```
...
### Step 2: Data Copying Test ($Z_U$)

Navigate to `DataCopying/` to calculate the $Z_U$ score. This metric determines if the generative model is overfitting (memorizing training data).

**Usage:**

```bash
cd DataCopying
# Inputs are the JSON files from the retrieval step containing distance lists
python datacopying_test.py gen_retrieval_results.json test_retrieval_results.json
```

### Step 3: Fréchet Point Distance (FPD)

Navigate to `fpd_eval/` to measure generation quality. We use Uni3D features to compute the training FD and test FD. You can get Uni3D embedding database from `retrieval/` step.
