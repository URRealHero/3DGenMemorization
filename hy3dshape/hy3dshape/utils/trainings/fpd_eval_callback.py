# hy3dshape/utils/trainings/fpd_eval_callback.py
...
def _mesh_to_points(tri_verts: np.ndarray, tri_faces: np.ndarray, n_points: int, normalize_first: bool = False) -> np.ndarray:
    ...
    if not normalize_first:
        # keep current behavior: normalize after sampling
...
class FPDConfig:
    ...
    # NEW: multi-encoder support (preferred)
    encoders: Optional[List[Dict[str, Any]]] = None
    # Single encoder (still supported)
    encoder: Optional[Dict[str, Any]] = None

    # legacy PointNet args (kept for back-compat if no encoder(s) are provided)
    pointnet_ckpt: str = ""
    width_mult: int = 2
...
def _all_gather_features_object(local_feats: np.ndarray, device: torch.device) -> np.ndarray:
    # Safer for uneven shards: all_gather_object (host memory)
...
class ModelEvalFPDCallback(pl.Callback):
    ...
    def _run_eval_distributed(self, trainer: pl.Trainer, pl_module: pl.LightningModule, rank: int, world_size: int, tag: str):
        ...
        try:
            # 1) LVIS/class/text/image mapping/conds
            ...
            # 3) Sampler kwargs; translate octree depth if needed
            ...
            # 4) Init encoders (once per rank)
            if self._encoders is None:
                self._encoders = self._build_encoders(device)

            # 5) Generate once → point clouds
            ...
            # 6) For each encoder: encode locally, gather, load/build ref stats, compute FPD
            ...
            if dist:
                # make sure every rank observes if any rank errored
    ...
    def _ref_cache_paths(self, encoder, uids_all: List[str]) -> Tuple[str, str, str]:
        ...
        # Back-compat: old embeddings cache path (if you used that scheme already)
    ...
    def _load_or_build_ref_stats(self, uids_all: List[str], encoder) -> Tuple[np.ndarray, np.ndarray, int, str]:
        ...
        # Back-compat: convert existing embeddings cache into stats
    ...
    def _build_encoders(self, device):
        ...
        # ensure each encoder object exposes n_points (for downstream batching/downsampling)
        ...
        # decide a single sampling size for mesh→points step (generate once at the max)
