...
# --- FPS: prefer pointnet2_ops; fallback to a torch FPS if missing ---
try:
    ...
    @torch.no_grad()
    def fps(xyz: torch.Tensor, number: int) -> torch.Tensor:
        # naive O(B*N*number) GPU FPS; slower but fine for eval
        B, N, _ = xyz.shape
        device = xyz.device
...
class Group(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, color: torch.Tensor):
        ...
        # normalize neighborhoods by centers
...
class Encoder(nn.Module):
    ...
    def forward(self, point_groups: torch.Tensor) -> torch.Tensor:
        ...
        x = point_groups.reshape(B * G, M, 6).transpose(1, 2)  # (BG,6,M)
        x = self.first(x)                                      # (BG,256,M)
        g = torch.max(x, dim=2, keepdim=True)[0]              # (BG,256,1)
        x = torch.cat([g.expand(-1, -1, M), x], dim=1)        # (BG,512,M)
        x = self.second(x)                                    # (BG,C,M)
        g = torch.max(x, dim=2, keepdim=False)[0]             # (BG,C)
        return g.reshape(B, G, -1)

class PointcloudEncoder(nn.Module):
    def __init__(self, point_transformer: nn.Module, args):
        super().__init__()
        # args fields used below
        ...
        self.visual = point_transformer  # timm transformer

    def forward(self, pts: torch.Tensor, colors: torch.Tensor) -> torch.Tensor:
        # pts/colors: (B,N,3)
        _, centers, feats = self.group_divider(pts, colors)             # feats: (B,G,M,6)
        tokens = self.encoder(feats)                                     # (B,G,Cenc)
        tokens = self.encoder2trans(tokens)                              # (B,G,T)
        B, G, T = tokens.shape
        cls = self.cls_token.expand(B, -1, -1)
        cls_pos = self.cls_pos.expand(B, -1, -1)
        pos = self.pos_embed(centers)                                    # (B,G,T)
        x = torch.cat([cls, tokens], dim=1)                              # (B,1+G,T)
        pos = torch.cat([cls_pos, pos], dim=1)                           # (B,1+G,T)
        x = x + pos
        x = self.patch_dropout(x)

        # timm EVA blocks
        # expect attributes: pos_drop, blocks (ModuleList), norm, optional fc_norm
        ...
        x = self.trans2embed(x)                                          # (B,embed_dim)
        return x
