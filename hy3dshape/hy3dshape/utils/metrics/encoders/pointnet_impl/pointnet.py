# hy3dshape/utils/metrics/encoders/pointnet_impl/pointnet.py
...
try:
    # vendored model in this same package
...
def normalize_bbox_unit_sphere(pc: np.ndarray) -> np.ndarray:
    """
    Vectorized unit-sphere normalization per sample (N, P, 3).
      shifts = (v.max + v.min)/2
      v <- v - shifts
      scale = 1 / max(||v||)
    """
...
class PointNetEncoderLocal:
    """
    Local, no-network PointNet++ encoder compatible with Point-E’s classifier head.
    - Expects a local checkpoint file (e.g., pointnet.pt) containing 'model_state_dict'
      or a raw state_dict. If keys are prefixed with 'module.', we strip it.
    - Returns per-cloud features from the penultimate layer (same as Point-E FPD).
    """
    ...
    def __init__(
        ...
        """
        Args:
            ckpt_path: local .pt path (no network used).
            devices: list of devices; default = all visible GPUs or CPU.
            device_batch_size: per-device batch size.
            width_mult: must match the checkpoint (Point-E’s ckpt uses 2).
            normal_channel: pass True if your clouds include normals (we use XYZ only -> False).
            normalize: set True to apply unit-sphere normalization inside encoder (we keep False;
                       our caller normalizes before encoding).
        """
        ...
        for dev in self.devices:
            ...
            if missing or unexpected:
                # Usually harmless (classifier head mismatch is fine when we only read features)
    ...
    def encode_np(self, pcs: np.ndarray) -> np.ndarray:
        """
        pcs: (N, P, 3) float array. Returns features (N, D) float32.
        """
        ...
        # Process in interleaved multi-device mini-batches
        for start in range(0, n, self.device_batch_size * num_devices):
            ...
            # Run forward per device, collect features
