...
# --- IMPORTANT: You must create this file and port the MoEBlock code from Hunyuan3D. ---
...
class ConditionalDiTBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # --- Optional Timestep Modulation (unused in final architecture but kept for fidelity) ---
    ...
    def forward(self, x, c=None, text_states=None, skip_value=None):
        ...
        # 1. Handle the skip connection if enabled for this block
        if self.skip_connection:
            x = self.skip_linear(torch.cat([skip_value, x], dim=-1))
            x = self.skip_norm(x)

        # 2. Layer 1: Self-Attention
        if self.timested_modulate:
            shift_msa = self.default_modulation(c).unsqueeze(dim=1)
            x = x + shift_msa
        x = x + self.attn1(self.norm1(x))

        # 3. Layer 2: Cross-Attention
        x = x + self.attn2(self.norm2(x), text_states)

        # 4. Layer 3: Feed-Forward (MLP or MoE)
...
class UnconditionalDiTBlock(nn.Module):
    """
    A DiT block for unconditional models. Cross-attention is replaced
    with a second self-attention layer.
    """
    def __init__(
        self,
        hidden_size,
        ...
        **kwargs,
        ...
        # --- A SECOND Self-Attention (in place of Cross-Attention) ---
        self.norm2 = norm_layer(hidden_size, elementwise_affine=True, eps=1e-6)
        ...
        # --- Optional Timestep Modulation (unused but kept for fidelity) ---
        self.timested_modulate = timested_modulate
        if self.timested_modulate and c_emb_size is not None:
            self.default_modulation = nn.Sequential(
                nn.SiLU(),
    ...
    def forward(self, x, c=None, text_states=None, skip_value=None):
        ...
        # 1. Handle the skip connection if enabled
        if self.skip_connection:
            x = self.skip_linear(torch.cat([skip_value, x], dim=-1))
            x = self.skip_norm(x)
        ...
        # 3. Layer 2: Second Self-Attention
...
class DiTPlain(nn.Module):
    ...
    def forward(self, x, t, contexts):
        ...
        # --- sanity checks for modality-agnostic "text_len" (i.e., context length) ---
        if self.use_attention_pooling:
            ...
            # pooling off: allow any L, but still enforce channel dim if known
