...
# Use absolute imports to prevent breakage
...
class MultiConditioner(nn.Module):
    """
    Unified interface for Class, Text, and Image conditioning.
    """
    def __init__(
        self, 
        type: str,
    ...
    def forward(self, conditioning_data, **kwargs):
        ...
        Args:
            conditioning_data: 
                - Tensor[int] for class
                - List[str] or str for text
                - Tensor or List[PIL] for image
        """
        ...
        if self.type == "text":
            ...
            # Apply Dropout (replace with empty string)
            if self.training and self.p_uncond > 0.0:
        ...
        if self.type == "image":
            ...
            # Apply Dropout (zero out embedding)
            if self.training and self.p_uncond > 0.0:
