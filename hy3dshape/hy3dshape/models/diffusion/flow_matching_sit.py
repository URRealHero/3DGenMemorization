...
class Diffuser(pl.LightningModule):
    def __init__(
        ...
        *,
        ...
        if self.torch_compile:
            # IMPORTANT: assign the compiled module so it actually takes effect.
            # torch.nn.Module.compile(...) is equivalent to torch.compile(...) and returns a compiled module.
            # We keep defaults to avoid changing behavior.
    ...
    def init_from_ckpt(self, path, ignore_keys=()):
        ...
        # DeepSpeed / HF merged / raw state-dict cases:
        # normalize potential containers
    ...
    def on_fit_start(self) -> None:
        ...
        # 2) align LR schedule when doing weights-only resume (init_ckpt)
    ...
    def forward(self, batch):
        ...
        # 2) fetch and (optionally) scale latents (make device transfer explicit)
        with torch.autocast(device_type="cuda", dtype=self._autocast_dtype):
            with torch.no_grad():
                with nvtx("encode"):
