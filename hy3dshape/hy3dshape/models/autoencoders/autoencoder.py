...
class VecSetAutoEncoder(nn.Module):
    ...
    def learn_and_decode(self, bottleneck_output, queries):
        # This function now contains the chunking logic.
        
        # 1. Process the latent vector through the bottleneck and transformer layers.
        #    This part is fast and is done only once.
        ...
        # 2. Decode the queries in chunks to save memory.
    ...
    def forward(self, pc, queries):
        ...
        # The forward pass is now simpler, as it just calls the chunked learn_and_decode
...
class CustomShapeVAE(nn.Module):
    def __init__(
        self,
        *,
        # These are the arguments your VAE needs, passed from the main config file
        model_depth: int,
        model_dim: int,
        surface_size: int,
        num_latents: int,
        ...
        # Create a simple temporary object to pass arguments to your factory function
        class Args:
            pass
        ...
        # 1. Build the core autoencoder using your existing factory
        #    This keeps all your proven logic.
        self.autoencoder = create_autoencoder(args)
        
        # 2. Instantiate the generic decoder and extractor tools from utils.py
        #    These will be used for the final mesh generation.
    ...
    def encode(self, surface, sample_posterior=True):
        """
        This method is called to get latents for the diffusion model.
        'surface' is expected to be a point cloud of shape [B, N, 3] or more.
        """
        # Your encoder just takes the first 3 dims (xyz)
        pc = surface[..., :3]
        
        # Use your proven, existing encoder logic
    ...
    def decode(self, latents, queries):
        """
        This is the core implicit field decoder. It takes processed latents and
        query points and returns SDF values.
        """
        # We wrap the latents in a dictionary to match the expected input
        # of your 'learn_and_decode' function.
        bottleneck_output = {'x': latents}
        
        # Call your proven, existing decoder logic
        out = self.autoencoder.learn_and_decode(bottleneck_output, queries)
        return out.squeeze(-1)

    def latents2mesh(self, latents, **kwargs):
        """
        This is the main inference method. It orchestrates the full decoding
        from a latent vector to a final mesh.
        """
...
def create_autoencoder(args):
    """
    V3 factory function to create the autoencoder based on command-line arguments.
    """
    # 1. Select the bottleneck class based on the 'bottleneck' argument
    bottleneck_map = {
        'kl': KLBottleneck,
    ...
    # 2. Prepare arguments for the selected bottleneck
    bottleneck_args = {
        'dim': args.model_dim,
        'latent_dim': args.latent_dim,
    }
    
    # 3. Instantiate the bottleneck
    bottleneck_instance = bottleneck_cls(**bottleneck_args)

    # 4. Create the main VecSetAutoEncoder model with all configurations
