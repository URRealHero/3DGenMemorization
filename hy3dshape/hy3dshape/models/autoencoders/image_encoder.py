# hy3dshape/models/autoencoders/image_encoder/flux_vae.py
...
class FluxVAE(nn.Module):
    ...
    Contract:
      - encode(x) expects x in [-1, 1], shape (B,3,H,W); returns z_scaled = (z - shift) * scale
      - decode(z_scaled) inverts: z = z_scaled/scale + shift; returns x in [-1, 1]
      - By default runs in eval() and frozen (first-stage is inference-only in your training loop).

    Notes:
      * If you see numeric drift with bf16/fp16, set enforce_fp32=True.
      * This is resolution-agnostic (H,W multiple of 8). You can still hard-resize upstream if you want 256x256 only.
    """
    def __init__(
        self,
        pretrained_path: str,
    ...
    def encode(self, images: torch.Tensor, sample_posterior: bool = True) -> torch.Tensor:
        """
        images: (B,3,H,W) float in [-1,1] or [0,1]
        returns: z_scaled = (z - shift) * scale  (B,C,H/8,W/8)
        """
    ...
    def decode(self, z_scaled: torch.Tensor) -> torch.Tensor:
        """
        z_scaled: (B,C,h,w) expected to be (z - shift) * scale
        returns: reconstructed images in [-1,1], shape (B,3,H,W)
        """
    ...
    # Optional: convenience full-pass for testing
    @torch.no_grad()
...
if __name__ == "__main__":
    # simple test
    vae = FluxVAE(pretrained_path="/home/sp2526/flux/ckpt/vae").eval().cuda()
