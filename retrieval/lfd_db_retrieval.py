...
def worker(
    ...
    # Test features: keep on CPU to minimize VRAM footprint; move per query
    ...
    # micro-batch queries to improve occupancy (per-query tensors are tiny)
...
def main():
    ...
    # Optional: show one sample key to prove content
    if len(check):
        k = next(iter(check))
