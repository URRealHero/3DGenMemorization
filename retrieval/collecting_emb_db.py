#!/usr/bin/env python3
"""
Consolidate single-vector .npy embeddings into one or many databases.

Discovery modes:
- By exact filename: --embedding_name point_embedding.npy
- By glob pattern:   --pattern "*.npy" (e.g. flattened dir with <db_root>/<model_uid>.npy)

Grouping:
- --keep_level 0: one DB for everything under --db_root
- --keep_level 1: one DB per immediate subdirectory of --db_root
- --keep_level 2: one DB per subdir/subdir, etc.

Each DB is saved as:
  {output_dir}/{output_prefix}__{group}.npy
  {output_dir}/{output_prefix}__{group}_meta.json
For keep_level=0, the group name is "root".

Model ID:
- Default: taken from the relative path component at --model_id_level
- With --model_id_from filename: taken from the file's stem (for flattened dirs like <db_root>/<model_uid>.npy)

New:
- --overwrite flag to force regeneration.
- Skip groups whose consolidated outputs already exist when --overwrite is not set.
- --pattern for glob-based discovery.
- --model_id_from to choose how model_id is derived.
"""
