...
class MeshEncoder:
    ...
    def align_mesh(self):
        ...
        copied_file_paths = [] # Keep track of successfully copied files for cleanup

        try:
            # 1. Copy necessary files using shutil
            for f_name in copy_file_names:
                ...
                try:
                    ...
                    raise  # Re-raise to be caught by the outer try-except in render_lfd

            # 2. Run the 3DAlignment executable
            ...
            process = subprocess.Popen(
                cmd,
                cwd=str(run_dir_path), # Popen cwd expects a string
            ...
            if process.returncode != 0:
                ...
                raise RuntimeError(error_message) # This will be caught by render_lfd

            # 3. Move/Rename generated files
            ...
            # 4. Guaranteed cleanup of copied files (and the input .obj created by MeshEncoder)
            ...
            # Clean up the input .obj file (self.temp_path is .../{self.file_name}.obj)
            # This file is created by MeshEncoder's __init__
            if self.temp_path.exists(): # self.temp_path is already a Path object
                try:
