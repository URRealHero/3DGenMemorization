...
Build lightweight LFD DBs (art/fd/cir/ecc + meta) from:
  ...
  B) a directory tree root (db_root) with grouping by keep_level.

Outputs per DB in <out_dir>/<group_relpath>/:
  - lfd_db_art.pt   [N,10,10,35] int
  - lfd_db_fd.pt    [N,10,10,10] int
  - lfd_db_cir.pt   [N,10,10]    int
  - lfd_db_ecc.pt   [N,10,10]    int
  - lfd_db_meta.json (one object per row: {"model_id","category_id","relpath"})

Grouping (db_root mode):
  We discover each *leaf* feature directory by finding any file matching
  `*{ART_SUFFIX}` (e.g., `_q8_v1.8.art`) anywhere under `--db_root` and
  using the file's parent directory. With `--keep_level=2`, grouping is by the
  first two components of the path relative to `--db_root` (e.g.,
  `Text-.../checkpoint-100K`). DBs are written under `<out_dir>/<group>/`.

Model ID selection:
  --model_id_level = K selects model_id as the name of the K-th ancestor
  of the leaf feature directory (K=0 → leaf name, K=1 → parent name, etc.).
  Category (when --infer_shapenet) is taken as one ancestor *above* the
  chosen model_id node when available.

Robustness:
  Loads each feature dir in an isolated child process (default) to avoid
  crashing the whole run if any single dir is corrupt.

Overwrite/skip:
  If a target DB directory already appears to contain a built DB
  (determined by the presence of lfd_db_meta.json), we skip it unless
...
def _child_load_one(dir_path: Path, device: str, out_npz: Path):
    ...
    # warm up globals (if this crashes, child exits non-zero)
...
def build_db_for_dirs(
    ...
    *,
    ...
    if not isolate:
        # Fast path (may crash if any dir is corrupt)
        loader = LFDBatchLoader(device=device)
        _ = loader.get_global_tensors()
...
def discover_feature_dirs(db_root: Path) -> List[Path]:
    ...
    returning their parent directories (deduped). This avoids assuming a fixed
...
def main():
    ...
    if not do_group:
        ...
        # Group by the first K components relative to db_root
